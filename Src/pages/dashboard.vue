  ...
  <div class="min-h-screen flex flex-col items-center justify-start bg-gradient-to-b from-orange-500 to-orange-800">
    ...
    <div class="w-full flex justify-between items-center p-4 bg-green-900 text-white shadow-lg fixed top-0 left-0 z-50">
      ...
      <div class="relative flex items-center space-x-4">
        ...
        <!-- Menu dropdown -->
        <div v-if="menuOpen" class="absolute right-0 mt-10 w-48 bg-white text-black rounded shadow-lg z-50">
          <button class="w-full text-left px-4 py-2 hover:bg-gray-200" @click="goToRegister">Cadastrar Usuário</button>
          <button class="w-full text-left px-4 py-2 hover:bg-gray-200" @click="goToEdit">Alterar meu cadastro</button>
          <button class="w-full text-left px-4 py-2 hover:bg-gray-200" @click="logout">Sair</button>
        </div>
      </div>
    </div>

    <!-- Espaço extra para não sobrepor conteúdo -->
    <div class="h-32"></div> <!-- mais espaçamento entre navbar e lista -->

    <!-- Lista de usuários maior -->
    <div class="p-8 border rounded shadow-md w-full max-w-4xl bg-white">
      <h2 class="text-2xl font-semibold mb-6 text-green-900">Lista de Usuários</h2>
      <ul class="text-lg">
        <li v-for="u in users" :key="u.id" class="mb-3 border-b py-2">
          {{ u.name }} - {{ u.email }}
        </li>
      </ul>
      <p v-if="users.length === 0" class="text-gray-500 text-lg">Nenhum usuário cadastrado.</p>
    </div>
...
const goToEdit = () => {
  menuOpen.value = false;
  router.push("/edit-profile"); // rota para alterar cadastro
};
...
const loadUsers = async () => {
  try { users.value = await AuthService.getUsers(); } 
